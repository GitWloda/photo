...
hash_similarity() {
    local a="$1"
    local b="$2"
    local ha hb dist total_bits sim
    ...
    sim="$(
        awk -v d="$dist" -v t="$total_bits" 'BEGIN {
            v = 1 - (d / t)
            if (v < 0) v = 0
            if (v > 1) v = 1
            printf "%.6f", v
...
if (( pair_total == 0 )); then
    info "Non ci sono abbastanza frame per confronti di similarità."
    exit 0
fi

info "Configurazione similarità:"
