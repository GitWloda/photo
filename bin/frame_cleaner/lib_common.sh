...
ROOT_DIR_FC="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
CONFIG_FILE_DEFAULT="$ROOT_DIR_FC/config/frame_cleaner.conf"
...
load_config() {
    ...
    if [[ -f "$config_file" ]]; then
        ...
        echo "[WARN] Config non trovata: $config_file — uso valori di default" >&2
    fi

    DEFAULT_DIR="${DEFAULT_DIR:-/data/frame}"
...
require_command() {
    ...
    command -v "$cmd" >/dev/null 2>&1 || {
        echo "[ERR] '$cmd' non trovato nel PATH" >&2
        exit 1
    }
}
...
quality_score_common() {
    ...
    awk \
        ...
        -v wm="$QS_WEIGHT_MEAN" \
        -v wd="$QS_WEIGHT_DARK" \
        'BEGIN { print (s*ws) + (c*wc) + (m*wm) - (d*wd) }'
}
